...
require('dotenv').config();  // loads backend/.env (script runs from backend/)
...
async function setup() {
  ...
  try {
    ...
    for (const stmt of statements) {
      try {
        await conn.query(stmt);  // use query() not execute() — DDL doesn't support prepared statements
        success++;
      } catch (err) {
    ...
    // Update .env with the correct password (already done but ensure it's saved)
